...
for i=1:length(LatentNodes)
    MI_Ourmethod{i}=[];
    for j=1:length(Neighbor_Ourmethod{i})
        tmp_corr=corrcoef(Data(:,Neighbor_Ourmethod{i}(j)),latent(LatentNodes(i),:)');
       MI_Ourmethod{i}=[MI_Ourmethod{i},tmp_corr(1,2)];
    end
end
%%%
...
for i=1:size(TF_list,1)
    MI_ARACNE{i}=[];
    for j=1:length(Neighbor_ARACNE{i})
        tmp_corr=corrcoef(Data(:,TF_list{i,3}), Data(:,Neighbor_ARACNE{i}(j)));
        MI_ARACNE{i}=[MI_ARACNE{i},tmp_corr(1,2)];
    end
end
save('global_CORRscore.mat');
exit;
